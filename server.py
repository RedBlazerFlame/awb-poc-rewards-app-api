...
def map_user_info_fields(user_info):
    ...
    fields_to_get = list(map(lambda field_name: field('user_info', field_name), fields_to_map))
    return {new_name: user_info[old_name] for (new_name, old_name) in zip(fields_to_map, fields_to_get)}
...
def get_by_field(model, field, field_value, fields_to_get):
    return query(model, 'search_read', [[(field, '=', field_value)]], {'fields': fields_to_get})

def get_users_by_field(field, field_value, fields_to_get):
    return get_by_field(full_model_name('user_info'), field, field_value, fields_to_get)
...
def get_user_info():
    ...
    fields_to_get = field_l('user_info', ['username', 'phone_number', 'email', 'customer_id'])
    res = get_users_by_field(field('user_info', 'customer_id'), customer_id, fields_to_get)
    return list(map(map_user_info_fields, res))
...
def update_user_by_customer_id():
    ...
    fields_to_get = field_l('user_info', ['username', 'phone_number', 'email', 'customer_id'])
    customer = get_users_by_field(field('user_info', 'customer_id'), session['customer_id'], fields_to_get)
    if(len(customer) == 0):
    ...
    if(email is not None and count_users_by_field(field('user_info', 'email'), email) > 0):
        return "Email is not unique", 400
    
    if(username is not None and count_users_by_field(field('user_info', 'username'), username) > 0):
        return "Username is not unique", 400
    
    if(phone_number is not None and count_users_by_field(field('user_info', 'phone_number'), phone_number) > 0):
        return "Phone number is not unique", 400

    res = query(full_model_name('user_info'), 'write', [[customer["id"]], {(field('user_info', field_name)): request.json.get(field_name) for field_name in filter(lambda field_name: request.json.get(field_name) is not None, fields_to_update)}])
...
def sign_up_user():
    ...
    validation_result = Validators.multi_validator(
        [
            Validators.username_check(request.json.get('username'), perform_none_check=True),
            Validators.phone_number_check(request.json.get('phone_number'), perform_none_check=True),
            Validators.email_check(request.json.get('email'), perform_none_check=True),
            Validators.password_check(request.json.get('password'), perform_none_check=True),
        ]
    )
    ...
    if(count_users_by_field(field('user_info', 'phone_number'), phone_number) > 0):
        return "Phone number is not unique", 400
...
def log_in_user():
    ...
    password = request.json.get('password')
    res = []
    fields_to_get = field_l('user_info', ['password', 'account_type', 'customer_id'])
    ...
    res = list(filter(lambda entry: entry[field('user_info', 'account_type')] == 'normal', res))
